...
import {
  adicionarAndamentoRemoto,
  gerenciarProtocoloRemoto,
  moverStatusRemoto,
} from "@/lib/protocolos-client";

const PASSOS_DETALHES: TourStep[] = [
  {
    alvo: '[data-tour="detail-header"]',
    titulo: "Detalhes do protocolo",
    texto: "Esta página reúne todas as informações e ações de um protocolo.",
    posicao: "bottom",
  },
  {
    alvo: '[data-tour="detail-identity"]',
    titulo: "Identificação e situação",
    texto:
      "Confira o número, a etapa atual e alertas importantes do protocolo.",
    posicao: "bottom",
  },
  {
    alvo: '[data-tour="detail-data"]',
    titulo: "Dados da solicitação",
    texto:
      "Aqui estão os dados do solicitante, tipo do pedido e datas de movimentação.",
    posicao: "bottom",
  },
  {
    alvo: '[data-tour="detail-controls"]',
    titulo: "Gestão do protocolo",
    texto:
      "Acompanhe o prazo, atribua um responsável e mova o protocolo de etapa.",
    posicao: "bottom",
  },
  {
    alvo: '[data-tour="detail-history"]',
    titulo: "Histórico completo",
    texto: "Consulte todas as movimentações, mensagens e documentos anexados.",
    posicao: "top",
  },
  {
    alvo: '[data-tour="detail-internal-notes"]',
    titulo: "Notas internas",
    texto: "Registre observações visíveis apenas para a equipe administrativa.",
    posicao: "right",
  },
  {
    alvo: '[data-tour="detail-manual-update"]',
    titulo: "Novo andamento",
    texto: "Adicione mensagens e anexos que ficarão visíveis ao solicitante.",
    posicao: "left",
  },
];
...
const PASSOS_DETALHES_ARQUIVADO = PASSOS_DETALHES.filter((passo) =>
  [
    '[data-tour="detail-header"]',
    '[data-tour="detail-identity"]',
    '[data-tour="detail-data"]',
    '[data-tour="detail-history"]',
  ].includes(passo.alvo),
);
...
export function ProtocoloDetalhes({
  ...
  useEffect(() => {
    ...
    async function carregarResponsaveis() {
      ...
      try {
        const resposta = await fetch("/api/admin/protocol-assignees", {
          cache: "no-store",
        });
        const dados = await resposta.json().catch(() => null);
        if (!resposta.ok) {
          throw new Error(
            dados?.error ?? "Nao foi possivel carregar responsaveis.",
          );
        }
    ...
    if (acaoPendente.tipo === "andamento") {
      try {
        ...
        toast(
          error instanceof Error
            ? error.message
            : "Nao foi possivel registrar o andamento.",
        );
      }
      ...
      try {
        ...
        toast(
          error instanceof Error
            ? error.message
            : "Nao foi possivel atualizar o protocolo.",
        );
      }
    } else if (acaoPendente.tipo === "responsavel") {
      try {
        await gerenciarProtocoloRemoto(
          protocolo.id,
          "assign",
          acaoPendente.responsavelId,
        );
      } catch (error) {
        toast(
          error instanceof Error
      ...
      try {
        ...
        toast(
          error instanceof Error
            ? error.message
            : "Nao foi possivel desarquivar o protocolo.",
        );
      }
    } else {
      try {
        ...
        toast(
          `Resposta recusada. Envie um andamento para notificar ${protocolo.nome}.`,
        );
      } catch (error) {
        toast(
          error instanceof Error
            ? error.message
            : "Nao foi possivel recusar a resposta.",
        );
      }
    }
    setAcaoPendente(null);
  ...
  const responsavelAtualForaDaLista =
    ...
    !responsaveis.some(
      (responsavel) => responsavel.name === protocolo.responsavel,
    );
  ...
  return (
    <section className="flex h-dvh min-h-0 flex-col overflow-hidden bg-background">
      <header
        data-tour="detail-header"
        className="shrink-0 border-b border-border bg-card px-6 py-6 lg:px-8"
      >
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 max-w-2xl text-sm leading-relaxed text-muted-foreground">
              Consulte os dados da solicitação, acompanhe o histórico e registre
              novos andamentos.
            </p>
          </div>
        </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain pb-6">
        <div
          data-tour="detail-identity"
          className="mx-4 mt-4 flex flex-wrap items-center gap-3 rounded-xl border border-border bg-card px-6 py-4 shadow-sm lg:mx-8"
        >
        ...
        <dl
          data-tour="detail-data"
          className="mx-4 mt-4 grid gap-px overflow-hidden rounded-xl border border-border bg-border shadow-sm sm:grid-cols-3 lg:mx-8"
        >
          {[
            ...
            <div key={k} className={cnDetalhe(k === "Última movimentação")}>
              <dt className="text-xs font-bold uppercase tracking-wider text-muted-foreground">
                {k}
              </dt>
              <dd className="mt-0.5 text-sm font-semibold break-words text-foreground">
                {v}
              </dd>
            </div>
          ))}
        ...
        {!protocolo.arquivado ? (
          <div
            data-tour="detail-controls"
            className={`mx-4 mt-4 grid gap-px overflow-hidden rounded-xl border border-border bg-border shadow-sm ${coordenador ? "md:grid-cols-1" : "md:grid-cols-3"} lg:mx-8`}
          >
            <div className="bg-card px-6 py-4">
              ...
              <p className={cnPrazo(atrasado)}>
                {prazo
                  ? formatarData(prazo.toISOString())
                  : "Sem previsão calculada"}
              </p>
                ...
                <div className="bg-card px-6 py-4">
                  ...
                  {carregandoResponsaveis ? (
                    ...
                    <Select
                      ...
                      value={
                        responsaveis.find(
                          (item) => item.name === protocolo.responsavel,
                        )?.id ?? ""
                      }
                      onChange={(e) => {
                        const escolhido = responsaveis.find(
          ...
          <div className="mx-4 mt-4 flex flex-wrap items-center gap-4 rounded-xl border border-border bg-secondary/40 px-6 py-4 shadow-sm lg:mx-8">
            <div className="min-w-0 flex-1 basis-72">
              ...
              <p className="mt-1 text-sm font-semibold text-foreground">
                {protocolo.arquivadoEm
                  ? formatarData(protocolo.arquivadoEm)
                  : "Data não informada"}{" "}
                · {protocolo.arquivadoPor ?? "Responsável não informado"}
              </p>
            ...
            {!coordenador && (
              <button
                type="button"
                onClick={() => setAcaoPendente({ tipo: "desarquivar" })}
                className="inline-flex items-center gap-2 rounded-full bg-primary px-5 py-2.5 text-sm font-extrabold text-primary-foreground hover:opacity-90"
              >
            ...
            <div className="mx-4 mt-4 flex flex-wrap items-center gap-3 rounded-xl border border-[#B7791F]/40 bg-[#B7791F]/8 px-6 py-4 shadow-sm lg:mx-8">
              <div className="min-w-0 flex-1 basis-72">
                ...
                <p className="mt-0.5 text-xs leading-relaxed text-muted-foreground">
                  Confira o anexo no histórico e aprove para retomar a
                  tramitação ou solicite uma nova correção ao aluno.
                </p>
              </div>
              {!coordenador && (
                <>
        ...
        <div
          data-tour="detail-history"
          className="mx-4 mt-4 rounded-xl border border-border bg-card px-6 py-5 shadow-sm lg:mx-8"
        >
        ...
        {(!protocolo.arquivado || coordenador) && (
          <div
            className={`mx-4 mt-4 grid items-stretch gap-4 ${coordenador ? "xl:grid-cols-1" : "xl:grid-cols-2"} lg:mx-8`}
          >
            {/* Notas internas — visíveis apenas para a equipe */}
            <div
              data-tour="detail-internal-notes"
              className="flex h-full flex-col rounded-xl border border-border bg-card px-6 py-5 shadow-sm xl:h-[36rem]"
            >
              <div className="min-h-20">
                ...
                <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
                  Registre observações exclusivas da equipe que não aparecem na
                  consulta pública.
                </p>
              ...
              {notas.length > 0 ? (
                ...
                <p className="mt-3 text-xs font-semibold text-muted-foreground">
                  Nenhuma nota interna registrada. Use este espaço para
            ...
            {!coordenador && (
              <div
                data-tour="detail-manual-update"
                className="flex h-full flex-col rounded-xl border border-border bg-card px-6 py-5 shadow-sm xl:h-[36rem]"
              >
                <div className="min-h-20">
                  <h3 className="text-base font-extrabold text-foreground">
                  ...
                  <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
                    A mensagem e os anexos ficam visíveis ao solicitante na
                    consulta pública, sem alterar a etapa atual. Uma notificação
                    de e-mail é simulada ao salvar.
                  </p>
                </div>
      ...
      <ConfirmacaoModal
        ...
        textoConfirmar={
          acaoPendente?.tipo === "recusar"
            ? "Solicitar correção"
            : acaoPendente?.tipo === "desarquivar"
              ? "Desarquivar"
              : "Confirmar ação"
        }
      ...
      <TourGuiado
        aberto={tourAtivo}
        passos={
          protocolo.arquivado ? PASSOS_DETALHES_ARQUIVADO : PASSOS_DETALHES
        }
...
function tituloConfirmacao(acao: AcaoPendente | null) {
  ...
  if (acao?.tipo === "responsavel")
    return "Confirmar alteração de responsável?";
...
function descricaoConfirmacao(acao: AcaoPendente | null) {
  if (acao?.tipo === "recusar")
    return "O aluno será notificado e deverá enviar uma nova documentação.";
  if (acao?.tipo === "status")
    return `O protocolo será movido para “${acao.status}”.`;
  if (acao?.tipo === "responsavel")
    return `O responsável será alterado para “${acao.responsavelNome || "Sem responsável"}”.`;
  if (acao?.tipo === "desarquivar")
    return "O protocolo voltará para a coluna correspondente à sua última etapa.";
  if (acao?.tipo === "nota")
    return "A nota ficará visível somente para a equipe administrativa.";
